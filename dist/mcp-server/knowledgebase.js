...
async function searchWebForMortgageInfo(question) {
    try {
...
export async function createKnowledgeBase() {
    console.log("✓ Knowledge base initialized (fast mode with web search fallback)");
    return {
        async answer(question) {
            ...
            // Fallback to general insights first
            const keywords = question.toLowerCase().split(/\W+/).filter(w => w.length > 3);
            const relevantInsights = WEB_INSIGHTS.filter(insight => keywords.some(kw => insight.toLowerCase().includes(kw)));
